...
public class PoiUtil {
     ...
     *
     * @param file
     * @param readSheet 读取第一个Sheet
     * @param startRow  读取起始行
     * @return
     * @throws IOException
     */
    public static List<String[]> readExcel(MultipartFile file, Integer readSheet, Integer startRow) throws IOException {
        // 检查文件
        checkFile(file);
        // 获得Workbook工作薄对象
        ...
        if (workbook != null) {
            // 获得当前sheet工作表
            // 默认只读取第一张表
            Sheet sheet = workbook.getSheetAt(!ValidateUtil.isEmpty(readSheet) ? readSheet : 0);
            if (sheet == null) {
                return list;
            }
            // 获得当前sheet的开始行
            ...
            int lastRowNum = sheet.getLastRowNum();
            // 循环除了第一行的所有行
            for (int rowNum = firstRowNum + (!ValidateUtil.isEmpty(startRow) ? startRow : 0); rowNum <= lastRowNum; rowNum++) {
                // 获得当前行
