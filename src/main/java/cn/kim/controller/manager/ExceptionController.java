...
public class ExceptionController extends BaseController {
    ...
    public ModelAndView sqlException(SQLException ex, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!ValidateUtil.isEmpty(AuthcUtil.getCurrent()) && SessionUtil.sqlException()) {
            //记录SQL错误日志
            LogUtil.recordLog(request, "SQL运行错误", UseType.SYSTEM.getType(), SystemEnum.SYSTEM.toString(), "SQL运行错误超过上限错误已被退出系统!", Tips.LOG_ERROR);
            ...
            //记录SQL错误日志
            LogUtil.recordLog(request, "SQL运行错误", UseType.SYSTEM.getType(), SystemEnum.SYSTEM.toString(), "SQL运行错误:" + ex.getMessage(), Tips.LOG_ERROR);
