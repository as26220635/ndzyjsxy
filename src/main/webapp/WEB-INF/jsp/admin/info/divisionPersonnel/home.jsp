    ...
    $dataGridTable.find('tbody').on('click', '#edit', function () {
        ...
        ajax.getHtml('${BASE_URL}${Url.DIVISION_PERSONNEL_UPDATE_URL}/' + id, {}, function (html) {
                model.show({
                    ...
                    content: html,
                    footerModel: model.footerModel.ADMIN,
                    <shiro:hasPermission name="INFO:DIVISION_PERSONNEL_UPDATE_SAVE">
                    isConfirm: true,
                    confirm: function ($model) {
                        var $form = $('#addAndEditForm');
                        //验证
    ...
    $dataGridTable.find('tbody').on('click', '#resetPassword', function () {
        ...
        model.show({
            ...
            confirm: function ($model, password) {
                ajax.put('${BASE_URL}${Url.DIVISION_PERSONNEL_RESET_PWD_URL}', {ID: id, PASSWORD: password}, function (data) {
                    ajaxReturn.data(data, $model, null, null);
                });
            }
        });
